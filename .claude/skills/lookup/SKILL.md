...
1. **질문 4개 완료** — 한 번에 1개씩, AskUserQuestion 사용
2. **추천 제시** — 출력 형식 준수, 다른 방식 4개 전부 근거 제시
3. **사용자 승인** — AskUserQuestion으로 승인 확인
4. **기록 저장 + 커밋** — `docs/lookups/YYYY-MM-DD-<주제>.md`
5. **다음 단계 안내** — 추천 방식별 테이블 참조
...
사용자가 문제를 입력하면, 아래 4가지를 **한 번에 1개씩** AskUserQuestion 객관식으로 질문한다.
...
반드시 아래 형식을 따른다. **다른 방식 4개 전부** 근거를 제시해야 한다 (1개라도 빠뜨리면 안 된다):
...
<HARD-GATE>
추천을 사용자가 승인하기 전에 기록 저장을 하지 않는다.
AskUserQuestion으로 승인 여부를 확인한다:
- A) 이 추천이 맞다, 진행하자
- B) 다시 분석하고 싶다
"다시 분석" → 1단계 질문으로 돌아간다.
</HARD-GATE>

사용자가 승인하면, 분석 결과를 파일로 저장하고 git commit한다.
...
| 이렇게 하면 안 된다 | 왜 |
|---|---|
| 질문 없이 바로 추천으로 점프 | 뻔해 보여도 질문해야 의외의 답이 나옴 |
| 질문 4개를 한꺼번에 던짐 | 한 번에 1개씩. AskUserQuestion 사용 |
| "다른 방식이 안 되는 이유"에서 일부 방식 생략 | 4개 전부 근거를 제시해야 함 |
| 추천하면서 구현 코드를 같이 작성 | 추천 + 근거만. 구현은 다음 단계의 몫 |
| 승인 없이 기록 파일 저장 | AskUserQuestion으로 승인 확인 필수 |
| 기록 파일 저장을 건너뜀 | 기록이 남아야 추적 가능 |
| 다음 단계 안내를 일반적으로 작성 | 4단계 테이블의 구체적 도구/명령어를 따름 |
| 다음 단계 안내 없이 종료 | 사용자가 다음에 뭘 해야 하는지 알아야 함 |

## 핵심 원칙

- **한 번에 질문 1개** — AskUserQuestion 객관식
- **추천만 하고, 구현은 하지 않는다** — 다음 단계의 몫
- **다른 방식 4개 전부 근거 제시** — 1개라도 빠뜨리지 않음
- **승인 후 기록** — 저장 전 반드시 사용자 확인
- **다음 단계는 구체적으로** — 4단계 테이블 참조
